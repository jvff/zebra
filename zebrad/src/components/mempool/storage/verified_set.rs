...
use super::super::StorageRejectionError;
...
impl VerifiedSet {
    ...
    /// same nullifier.
    pub fn insert(&mut self, transaction: UnminedTx) -> Result<(), StorageRejectionError> {
        if self.has_spend_conflicts(&transaction) {
            return Err(StorageRejectionError::SpendConflict);
        }
