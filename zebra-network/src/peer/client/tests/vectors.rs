...
/// Force the connection background task to panic, and check if the `Client` properly handles it.
#[tokio::test]
async fn client_service_handles_panicked_connection_task() {
    zebra_test::init();
...
async fn client_service_handles_exited_heartbeat_task() {
    ...
    handle.stop_heartbeat_task().await;
