...
/// Test if a V5 transaction with Sapling outputs but no spends is accepted by the verifier.
///
/// The fake transaction used here is checked to have a transparent input, to avoid finding a fake
/// transaction that has no inputs because it's based on a V4 transaction with Sprout joinsplits as
/// inputs.
// TODO: Remove `should_panic` once V5 transaction verification is complete.
// #[should_panic]
#[test]
fn v5_with_sapling_outputs_and_no_spends() {
    zebra_test::init();
    zebra_test::RUNTIME.block_on(async {
        let network = Network::Mainnet;
